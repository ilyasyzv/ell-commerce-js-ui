...
import {CartOrderSummary, CartProduct, CartProductHeader, CartQty, EmptyCart} from "../components";
...
const Template: ComponentStoryFn<typeof CartOrderSummary> = () => {
    ...
    return <>
            <StyledButtons>
                <button onClick={() => prepareData()} disabled={isLoading}>Prepare Data for all product </button>
                <br/>
                <button onClick={() => prepareData(1)} disabled={isLoading}>1 product </button>
                <br/>
                 <button onClick={() => prepareData(3)} disabled={isLoading}>3 product </button>
            </StyledButtons>
...
const ObserverComponent:FC<IObserverComponentsProps> = observer(({store}) => {
    const [isLoading, setIsLoading ] = useState(false)
    ...
    return <StyledContainer>
        {!store.cart || !store.cart.items.length ? <EmptyCart onBackButtonClick={()=>{}}></EmptyCart> :
            <>
                <CartQty className="header" itemsCount={store.cart?.items.length || 0}></CartQty>
                ...
                <CartOrderSummary cart={store.cart} onCheckoutClick={onCheckout}></CartOrderSummary>
            </>}
